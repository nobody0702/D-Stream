...
int main()
{
    freopen("data_shenmegui.txt","r",stdin);
    ...
    while(read_record(record_x)){
        ...
        if(tc==tgap)//如果到下一次更新时间点
        {
            ...
            learn_new_parameters(s);//学习下一个gap的参数：Cm，gap。
            tgap+=gap;
        }
    }
    return 0;
...
/*学习新的参数：Cl，Cm，gap*/
void learn_new_parameters(long double ss)
{
    Cl = (long double)2.0/3.0*1.0/(N*(1-LAMBDA));
    Cm = (long double)(1.0/(N*(1-LAMBDA))+2*ss)/3.0;
    long double tmp=(N*(1-LAMBDA)*Cm-1)/(N*(1-LAMBDA)*Cl-1);
    if(tmp <= 0)gap = floor(log(Cl/Cm)/log(LAMBDA));
    else gap = min((LL)floor(log(Cl/Cm)/log(LAMBDA)),(LL)floor(log(tmp)/log(LAMBDA)));
}
/*更新所有格子的参数，并计算一个平均值s*/
long double update_all_grid_cal_v()
{
...
void init_parameters_and_data_structures()
{
    ...
    for(int i=0;i<DIMENSION;i++)
        UPPERBOUND[i] = 200;
    UPPERBOUND[0] = 400;UPPERBOUND[1] = 200;
    for(int i=0;i<DIMENSION;i++)
        GRID_LEN[i] = 2;//格子的长度
    GRID_LEN[0] = 3;GRID_LEN[1] = 3;
    for(int i=0;i<DIMENSION;i++)
    ...
    Cl = (long double)2.9/3.0*1.0/(N*(1-LAMBDA));
    Cm = (long double)1.0/(N*(1-LAMBDA));
    long double tmp=(N*(1-LAMBDA)*Cm-1)/(N*(1-LAMBDA)*Cl-1);
    if(tmp <= 0)gap = floor(log(Cl/Cm)/log(LAMBDA));
    else gap = min(floor(log(Cl/Cm)/log(LAMBDA)),floor(log(tmp)/log(LAMBDA)));
